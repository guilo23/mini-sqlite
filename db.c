...
/* Methods to wright and read internal nodes */
...
uint32_t* internal_node_child(void* node,uint32_t child_num){
  ...
  if(child_num > num_keys){
    printf("Tried to access child_num %d > nun_keys %d\n",child_num,num_keys);
    exit(EXIT_FAILURE);
  } else if (child_num == num_keys) {
    uint32_t* right_child = internal_node_right_child(node);
    if(*right_child == INVALID_PAGE_NUM){
      printf("Tried to acess right child of node,but was invalid page\n");
      exit(EXIT_FAILURE);
    }
    return right_child;
  } else {
    ...
    if (*child == INVALID_PAGE_NUM) {
      printf("Tried to acess child %d of node,but was invalid page\n",child_num);
      exit(EXIT_FAILURE);
    }
    return child;
  }
 ...
 * go onto teh end of the base file 
 * */
...
uint32_t internal_node_find_child(void* node,uint32_t key){
  //Return the index of the child wich should contain
  //the given key.
  uint32_t num_keys = *internal_node_num_keys(node);
...
void leaf_node_split(Cursor* cursor,uint32_t key,Row* value){
  ...
  *(leaf_node_num_cells(new_node)) = LEAF_NODE_RIGHT_SPLIT_COUNT;
  ...
  uint32_t old_max_before_split = old_max;
  uint32_t new_max_after_split = get_node_max_key(cursor->table->pager, old_node);

  // Atualiza a chave do nó antigo no pai
  update_internal_node_key(parent, old_max_before_split, new_max_after_split);
...
void internal_node_split_and_insert(Table* table,uint32_t parent_page_num,
  ...
  */
  ...
  if(splitting_root){
    ...
    /*
    If we are splitting the root, we need to update old_root_node to pointers
    to the new root left child ,new page num will already point to 
    the new root right child
    */
  ...
  uint32_t current_page_num = *internal_node_right_child(old_node);
  void* current = get_page(table->pager,current_page_num);
 
  /*
    First put right child into new node and set right child of old node to invalid page number
  */
  internal_node_insert(table, new_page_num, current_page_num);
  *node_parent(current) = new_page_num;
  *internal_node_right_child(old_node) = INVALID_PAGE_NUM;
  /*
   For each key until you get to the meadle key, move the key and child to the new child
  */ 
  for(int i = *old_num_keys - 1;i > INTERNAL_NODE_MAX_CELLS/2;i--){
    current_page_num = *internal_node_child(old_node,i);
    current = get_page(table->pager,current_page_num);
  ...
  /*
   Set child before middle key, wich is now the highesr key, to be node right child,
  ...
  *internal_node_right_child(old_node) = *internal_node_child(old_node,*old_num_keys - 1);
  ...
  /*
   * Determine wich of the two nodes after the split should contain the child to be inserted
...
void leaf_node_insert(Cursor* cursor,uint32_t key,Row* value){
  ...
  //  //node full
    leaf_node_split(cursor,key,value);
    return;
  }
